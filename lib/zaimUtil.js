...
*/
    ...
    function counterStream () {
        const
            tStream = new require('stream').Transform({
                objectMode: true
            }),
            cntPrms = new Promise((resolve, reject) => {
                ...
                tStream._transform = function (chunk, encoding, callback) {
                    try {
                        ...
                        console.log(`counter-- chunk:${c} count:${count}`);
                        callback();
                    } catch (err) {
                        reject(err);
                    }
    ...
    function zaimMoneyStream(cntxt, startDate, endDate) {
        const
            limitNum = 100,
            retStream = new require('stream').Transform({
                objectMode: true
            }),
        ...
        co(function* (){
            ...
            do{
                let strm;
                strm = moneyStrm.getPage(page);
                strm.pipe(counter.stream).pipe(retStream, false);
                count = yield counter.promise;
                console.log(`page-${page} count:${count}`);
                page += 1;
            } while(count === limitNum && page < 4);
            retStream.end();
        });
