...
module.exports = (() => {
    ...
    function _ArrayToStream(prms, pass) {
        /*
            prms is Promise for resolve array
    ...
    function getCategoryByStream(cntxt) {
        ...
        function _getCategoryStream(mode, pass) {
            return _ArrayToStream(
                getCategory(cntxt, mode)
                .then((ret) => {return ret.categories;}),
                pass
            )
            .on('error', (err) => {
        ...
        _getCategoryStream('income')
            .on('data', (chunk) => {
                pass.add(chunk.id);
            })
            .on('end', () => {
                _getCategoryStream('payment', pass)
    ...
    function getGenreByStream(cntxt) {
        /* mode指定を無視。
            何を指定しても同じだと思われるので。
        */
        return _ArrayToStream(
            getGenre(cntxt, undefined)
                .then((ret) => {return ret.genres;})
        );
    }

    function getCategoryDict(cntxt, ctgNames) {
        /* category - genres をMapで返す */
        const
            dict = new Map(),
            ctgStrm = getCategoryByStream(cntxt),
            gnrStrm = getGenreByStream(cntxt);
        ...
        function loadCtg() {
            return new Promise((resolve, reject)=>{
                ctgStrm.on('data', (ctg) => {
                    try {
                        ctg.uname = ctg.name;
                        if (ctgNames[ctg.id] !== undefined){
                            ctg.name = ctgNames[ctg.id];
                        }
                        dict.set(ctg.id, {
                            category: ctg,
                            genres: new Map()
        ...
        function loadGnr() {
            return new Promise((resolve, reject)=>{
                gnrStrm.on('data', (gnt) => {
                    try {
                        dict.get(gnt.category_id).genres.set(gnt.id, gnt);
                    } catch (err) {
                        reject(err);
                    }
                });
    ...
    function getAccount(cntxt, mode) {
        let url;
        if (mode !== 'payment' && mode !== 'income' && mode !== 'transfer' && mode !== undefined && mode !== null) {
    ...
    function _Url_for_getMoney(cntxt, startDate, endDate, limitNum, page) {
        ...
        console.log('request order:' + cntxt.order);
...
*/
    ...
    function makeZaim(accessInfo, params) {
        const cntxt = {},
            selfIF = {};

        cntxt.selfIF = selfIF;
        if (!accessInfo.consumerKey || !accessInfo.consumerSecret) {
            throw new Error('ConsumerKey and secret must be configured.');
        }
        cntxt.consumerKey = accessInfo.consumerKey;
        cntxt.consumerSecret = accessInfo.consumerSecret;
        cntxt.callback = accessInfo.callback;
        ...
        cntxt.order = (params.order === undefined) ? 'date' : params.order;
