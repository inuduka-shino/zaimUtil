...
*/
    ...
    function zaimMoneyStream(cntxt, startDate, endDate) {
        ...
        let page = 1,
            preStreamPrms = Promise.resolve(page);

        retStream._read = function () {
            console.log('read request:' + page);
            ...
            preStreamPrms.then((page) => {
                ...
                jStream = require('JSONStream').parse('money');
                jStream.on('data', (moneys) => {
                    const pushStat = retStream.push(moneys);
                    count += moneys.length;
                    console.log('data event:' + moneys.length);
                    console.log('push retStream - status:' +pushStat);
                });
                preStreamPrms = new Promise ((resolve) => {
                    jStream.on('end', () => {
                        console.log('jStream End event');
                ...
                req = genRequest(cntxt, url(page));
                req.on('response', (message) => {
                    console.log('response event for page-' + page);
                    message.pipe(jStream);
                });
                req.end(); // exec request
            });

            /*
            httpGet(cntxt, url(page)).then((data) => {
                const moneys = data.money;
                retStream.push(moneys);
                if (moneys.length < limitNum) {
                    retStream.push(null);
                }
            });
            page += 1;
