...
module.exports = (() => {
    ...
    function getMoney(cntxt, startDate, endDate) {
        ...
        url = money_url + '?' + querystring.stringify({
            mapping: 1,
...
*/
        ...
        return new Promise((resolve, reject) => {
            const moneys = [];
            (function loop(page) {
                httpGet(cntxt, url + '&page=' + page)
                    .then((data) => {
                        arrayConcat(moneys, data.money);
                        if (data.money.length === limitNum) {
                            page += 1;
                            loop(page);
                        } else {
                            resolve(moneys);
    ...
    function makeZaim(params) {
        ...
        cntxt.token = params.accessToken;
        cntxt.secret = params.accessTokenSecret;
