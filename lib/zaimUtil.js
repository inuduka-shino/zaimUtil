...
module.exports = (() => {
    ...
    function _getCategoryStream(cntxt, mode, pass) {
        ...
        const rStream = new stream.Readable({
            ...
            prms = getCategory(cntxt, mode).then((ret) => {
    ...
    function getCategoryByStream(cntxt) {
        ...
        const
            retStream = new stream.Transform({
                objectMode: true
            }),
            pass = new Set();

        retStream._transform = function (chunk, encode, cb) {
            retStream.push(chunk);
            cb();
        };
        retStream._flush = function (cb) {
            cb();
        };
    ...
    function getCategoryUtil(cntxt) {
        /* modeを'income','payment'指定でそれぞれリクエスト
           結果をマージして返す */
        const dict = new Map();
        return co(function* () {
            const ret = yield ['income', 'payment'].map((mode) => {
                return getCategory(cntxt, mode).then((ret) => {
                    return ret.categories;
                });
            });
            ret.forEach((catgs) => {
    ...
    function getCategoryDict(cntxt) {
        ...
        return co(function* () {
            ...
            const catgRet = yield ['income', 'payment'].map((mode) => {
                return getCategory(cntxt, mode).then((ret) => {
                    return ret.categories;
                });
            });
            catgRet.forEach((catgs) => {
                catgs.forEach((ctg) => {
                    dict.set(ctg.id, {
                        category: ctg,
                        genres: new Map()
...
*/
    ...
    function zaimMoneyStream(cntxt, startDate, endDate) {
        ...
        retStream._transform = function (chunk, enc, cb) {
            retStream.push(chunk);
            cb();
        };
        retStream._flash = function (cb) {
            cb();
        };
        co(function* (){
            let page = 1,
    ...
    function makeZaim(params) {
        ...
        return Object.assign(selfIF, {
            ...
            getCategoryUtil: getCategoryUtil.bind(null, cntxt),
